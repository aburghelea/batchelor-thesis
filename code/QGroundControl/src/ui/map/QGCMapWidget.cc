...
void QGCMapWidget::setWaypointManager (UASWaypointManager *wpManager) {
    currWPManager = wpManager;
}
...
void QGCMapWidget::updateWaypoint(int uas, Waypoint* wp)
{
    ...
    //f (firingWaypointChange == wp) return;
    // Currently only accept waypoint updates from the UAS in focus
    // this has to be changed to accept read-only updates from other systems as well.
    UASInterface* uasInstance = UASManager::instance()->getUASForId(uas);

    if (uas == -1 || uasInstance->getWaypointManager() == currWPManager)
    {
        qDebug () << "hello\n";
        ...
        if (((wp->getFrame() == MAV_FRAME_GLOBAL) || (wp->getFrame() == MAV_FRAME_GLOBAL_RELATIVE_ALT)) && wp->isNavigationType())
        {
            ...
            qDebug () << "We're good, this is a global waypoint\n";
            ...
            if (!waypointsToIcons.contains(wp))
            {
                ...
                if(currWPManager->color.isValid ()) {
                    wpColor = currWPManager->color;
                }
...
/**
 * Update a specific list of waypoints. This is e.g. necessary if the list order changed.
 * The UAS manager will emit the appropriate signal whenever updating the list
 * is necessary.
 */
void QGCMapWidget::removeWaypointList(QVector<Waypoint*>& waypoints)
{
    qDebug () << "deleting specific waypoints";
    ...
    waypoints.remove (0, waypoints.size ());
}
...
void QGCMapWidget::updateWaypointList(int uas)
{
    ...
    // this has to be changed to accept read-only updates from other systems as well.
    UASInterface* uasInstance = UASManager::instance()->getUASForId(uas);
    if (uas == -1 || (uasInstance && (uasInstance->getWaypointManager() == currWPManager)) )
    {
        // ORDER MATTERS HERE!
...
UASWaypointManager* QGCMapWidget::getWaypointManager() {
    return currWPManager;
}

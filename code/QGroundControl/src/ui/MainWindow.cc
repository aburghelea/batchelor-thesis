...
MainWindow::MainWindow(QWidget *parent):
    ...
    styleFileName(QCoreApplication::applicationDirPath() + "/style-mission.css"),
    autoReconnect(false),
    lowPowerMode(false)
{
    hide();
   ...
   // centerStack->addWidget(new UAVMissionControlWidget(centerStack));
...
void MainWindow::buildCommonWidgets()
{
    ...
    if(!uavWidget) {
        uavWidget = new QDockWidget(tr("UAV Mission Control"), this);
        uavWidget->setWidget(new UAVMissionControlWidget(this));
        uavWidget->setObjectName("UAV_MISSION_CONTROL_WIDGET");
        addTool(uavWidget, tr("UAV Mission Control"), Qt::BottomDockWidgetArea);
    }
...
void MainWindow::reloadStylesheet()
{
    ...
    qDebug() << "CSS to read from "<<styleFileName << "\n";
...
QPointer<QGCMapTool> MainWindow::getMapTool() {
    return mapWidget;
}

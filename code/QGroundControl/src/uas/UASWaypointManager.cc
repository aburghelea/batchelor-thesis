...
#include "uav/UAVMissionConfig.h"
...
Waypoint* UASWaypointManager::createWaypoint(bool enforceFirstActive)
{
    ...
    if(uav) {
        uav->uavArea.push_back (wp);
        qDebug () << uav->uavArea.size () << " waypoints for uav " << color.name ();
    }
...
void UASWaypointManager::removeWaypoint (Waypoint *wp) {
    removeWaypoint (waypointsEditable.indexOf (wp));
}
...
void UASWaypointManager::loadWaypoints(const QString &loadFile)
{
    ...
    else
    {
        ...
        mapWidget->setWaypointManager(this);

        /* Clear waypoints from the map widget */
        mapWidget->removeWaypointList(waypointsEditable);
        mapWidget->updateWaypointList(-1);
        ...
        while (!in.atEnd())
        {
            ...
            if(t->load(in))
            {
                ...
                /* Display waypoint on the map widget */
                mapWidget->updateWaypoint(-1, t);
            }
            else
            {
